...
final class VectorEmbeddingProviderFactory {
  ...
  VectorEmbeddingProvider forSource(String sourceId) {
    String sql = "SELECT p.provider_type, p.api_key_reference, p.model, p.dimensions, p.timeout_seconds, p.max_input_characters, p.api_endpoint "
    ...
    try (PreparedStatement statement = connectionProvider.getPreparedStatement(sql)) {
      statement.setString(1, sourceId); try (ResultSet result = statement.executeQuery()) {
        ...
        return new OpenAiEmbeddingProvider(result.getString(2), result.getString(3), result.getInt(4), result.getInt(5), result.getInt(6), result.getString(7));
      }
